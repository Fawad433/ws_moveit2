...
int main(int argc, char** argv)
{
  ...
  moveit::planning_interface::MoveGroupInterface move_group_interface(node, "myrobot_arm");
  auto moveit_visual_tools =
      moveit_visual_tools::MoveItVisualTools{ node, "arm_base_link", rviz_visual_tools::RVIZ_MARKER_TOPIC,
  ...
  auto target_pose = get_relative_pose(-0.1, 0.0, 0.1);
  ...
  target_pose = get_relative_pose(-0.1, 0.0, 0.1);
  ...
  */
  ...
  auto target_pose = get_relative_pose(-current_pose.pose.position.x-0.097172, -current_pose.pose.position.y-0.193269, -current_pose.pose.position.z+0.286618);
